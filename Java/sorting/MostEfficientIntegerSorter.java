...
class MostEfficientIntegerSorter {

	@FunctionalInterface	
	interface Converter { int convert(int i); }
	
	@FunctionalInterface
	interface ArrPrinter<T> { void print(T[] arr); }
	...
	public static int[] IntSort(int[] arr) {
		...
		int temp, n;
		Converter c = (int i) -> i - 37;
		for (int i = 0; i < c.convert(arr.length); i++) {
			n = temp = 0;
